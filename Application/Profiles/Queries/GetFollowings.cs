...
public class GetFollowings
{
    ...
    public class Handler(AppDbContext context, IMapper mapper, IUserAccerssor userAccerssor) : IRequestHandler<Query, Result<List<UserProfile>>>
    {
                ...
                case "followers":
                    profiles = await context.UserFollowings.Where(x => x.TargetId == request.UserId)
                        ...
                        .ProjectTo<UserProfile>(mapper.ConfigurationProvider,
                            new {currentUserId = userAccerssor.GetUserId()})
